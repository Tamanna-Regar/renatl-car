...
export const carsList = [
  ...
  },
  {
    ...
    price: 'Rs 2,300/day',
    image:
      'https://media.drive.com.au/obj/tx_q:50,rs:auto:1920:1080:1/driveau/upload/cms/uploads/tigipu2ys2draydyxmgq.jpg',
    specs: {
      model: 'i20 Sportz',
      rating: '4.3',
      fuelType: 'Petrol',
...
const normalizeCar = (car, index = 0) => {
  ...
  return {
    ...
    id: String(safeCar.id || `car-${index}-${Date.now()}`),
...
export default function Cars() {
      ...
      if (savedCars) {
        ...
        if (Array.isArray(parsedCars)) {
          ...
          return parsedCars.map((car, index) => {
            const normalized = normalizeCar(car, index);

            // Automatically unbook if rental period has expired
            if (
              normalized.isBooked &&
              normalized.bookedUntil &&
              currentTime > normalized.bookedUntil
            ) {
              normalized.isBooked = false;
              normalized.bookedUntil = null;
            }
  ...
  useEffect(() => {
    const loadFleetCars = () => {
      try {
        const updatedFleet = JSON.parse(
          localStorage.getItem('fleetVehicles') || '[]'
        );

        const formattedFleet = updatedFleet
          .filter((v) => {
            const fueltype = String(v?.type || '')
              .trim()
              .toLowerCase();
            ...
            normalizeCar(
              {
                ...
                // FIX: force to string so it matches normalizeCar's
                // string ids consistently (avoids "50" vs 50 mismatch)
                id: String(
                  car.id || `fleet-${index}`
                ),
  ...
  const handleRentNow = (carId) => {
    ...
    alert(
      'Car booked successfully for 3 days! It will automatically become available after 3 days.'
    );
  };
    ...
    >
      ...
      >
        {filteredCars.map(
          (car, index) => (
            <div
              // FIX: id + index combo guarantees a unique key
              // even in the unlikely event of a leftover clash
              key={`${car.id}-${index}`}
              style={{
                border:
                  '1px solid #ddd',
